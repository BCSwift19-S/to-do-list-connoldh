...
//    var toDoNotesArray = ["I should be certain to do all of the exercises at the end of the chapters before the exam",
//                          "Take my ideas to the school's venture competition", "Focus apps on empowerment for all"]
...
extension ViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        ...
        let noteToMNove = toDoNotesArray[sourceIndexPath.row]
        toDoArray.remove(at: sourceIndexPath.row)
        toDoNotesArray.remove(at: sourceIndexPath.row)
        toDoArray.insert(itemToMove, at: destinationIndexPath.row)
        toDoNotesArray.insert(noteToMNove, at: destinationIndexPath.row)
        saveDefaultsData()
    }
}
